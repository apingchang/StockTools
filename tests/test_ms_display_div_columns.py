...
def test_股利為0時_現金殖利率應為0_0():
    """【V0.9.5-goodinfo6+++ 邊界】現金股利 = 0（公司該年未配息）→ 現金殖利率 = 0.0%

    William 2026-06-26 14:18 新規則：「2026 漲利率要用現價去計算」
    舊邏輯：cash=0 → 殖利率 None（誤導為「該股無資料」）
    新邏輯：cash=0 → 殖利率 = cash/現價 = 0/現價 = 0.0%（表示「該年未配息」= 0%）

    參考：6219 2026 cash=0 → 殖利率 0.0% 表示「該年未配息」是合理語意
    """
    st_fetch_market._fetch_finmind_dividend = lambda codes, **kw: pd.DataFrame([
        {
            "股票代號": "9999",
            ...
            f"{CY - 2}現金股利": None,
            f"{CY - 2}股票股利": None,
            f"{CY}現金殖利率_goodinfo": 0.0,
            f"{CY - 1}現金殖利率_goodinfo": 3.0,
        }
    ])
    ...
    # 【V0.9.5-goodinfo6+++】cash=0 → 殖利率 = 0/50 = 0.0%（不是 None）
    assert row["今年現金殖利率(%)"] == 0.0, \
        f"現金股利 = 0 時殖利率應 = 0.0%，實際: {row['今年現金殖利率(%)']}"
