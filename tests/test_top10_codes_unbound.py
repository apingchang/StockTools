...
def test_run_pipeline_return_handles_top10_codes():
    ...
    assert re.search(pattern, content), (
        "❌ run_pipeline 結尾沒處理 top10_codes 未定義情況！\n"
        "應為：\n"
        "  try:\n"
    ...
    print("✅ run_pipeline return 用 try/except 處理 top10_codes")
...
def test_top10_codes_assignment_in_if_block():
    ...
    assert m, "❌ 找不到 top10_codes = df_sel.head(10) 賦值"

    # 賦值縮排應該是 8 個 space（在 if 區塊內）
    indent = m.group(1)
    assert len(indent) >= 8, (
        f"❌ top10_codes 賦值縮排只有 {len(indent)} 個空白\n"
        f"應在 if cfg.use_top10_backtest: 區塊內（8 個空白）"
    )
    print(f"✅ top10_codes 賦值在 if 區塊內（縮排 {len(indent)} 個空白）")
