...
1. 【模擬買賣回推一日與回推 N 日功能 (Backward Time Travel)】：
   - 新增 `paper_trading.py` 核心回溯機制：`rollback_portfolio` 與 `rollback_portfolio_by_days`。
   - 歷史重播精準重建持倉（Holdings Replay Reconstruction）：依據目標日期前之 `sim_trades` 重構 `sim_holdings`（股數、均成本、進場日、分數），並自動還原可用現金與清理快照。
   - 在「買賣紀錄」分頁新增「◀ 回推一日」與「⏪ 回推 N 日...」按鈕，並提供友善之指定日期選擇對話框（`RollbackDaysDialog`）。
   - 支援「買賣紀錄」列表右鍵快捷選單：右鍵點選任一歷史紀錄即可「⏪ 回推至此交易日」。
2. 【順向推進與回退連貫機制】：
   - 強化 `_advance_one_day_for`：當組合被回退至歷史日期時，點擊「▶ 推進一天」將自動推進至下一歷史交易日並使用歷史資料運算，允許使用者回溯修改買賣參數後逐日觀察 App 操作。
...
【背景】William 2026-07-07 01:08 用截圖回報 v6 仍失敗:
  ...
  3. down key 還是沒辦法 scroll 到最下面一個 item
  截圖顯示康和 6016、欣巴巴 9906 兩個 row 同時被黃色 highlight
...
【v7 簡單設計(單一真相 = <<TreeviewSelect>> + <FocusIn>)】
  ...
  3. 新增 _on_tree_focus_in handler、bind <FocusIn> 到 4 個 tree
     → tree 拿到 widget focus 時自動 focus_set + focus(children[0])
     → 不管用 mouse click 或鍵盤 tab 切到 tree、Up/Down 都立刻有效
...
【v5 簡單設計(回到 hover_<price> tag 系統)】
  ...
  3. <Enter> 自動 focus_set + focus(children[0])
     → keyboard focus 永遠在 tree、Up/Down 自動切 focus + scroll
  4. 不綁 <Up>/<Down>、Treeview browse mode 預設會處理
  5. <<TreeviewSelect>> 監聽 selection 變化、<<TreeviewSelect>> handler
     _on_tree_select_sync_hover 先清舊、再設新
...
class StrategyGUI(tk.Tk):
    ...
    def _build_manual_select_tab(self, parent):
        ...
        self._ms_tree.bind("<FocusIn>", self._on_tree_focus_in)
    ...
    def _ms_toggle_check(self, event):
        ...
        # 【V1.2.0-kb-focus-v5】Treeview browse mode click 會自動 selection_set、會觸發
        # <<TreeviewSelect>> 事件、我們在那個 handler 同步 hover_<price> tag
